...
class PlayerAgent(Agent):
    ...
    def _bleedout_check(self, valid):
        """If chip lead > max remaining forced losses, lock into fold/check/instant discard."""
        ...
        if self._running_pnl > max_bleed:
            # DISCARD: let normal discard logic run (keeps best pair, not blind [0,1])
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        # ── Bleed-out lock (now handles discard too) ─────────────────────
