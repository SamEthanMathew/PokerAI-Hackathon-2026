#!/usr/bin/env python3
"""
METAV5 (tuned) vs LambdaV1 and ALPHANiTV7
...
def play_hand_logged(env, a0, a1, logf, match_num, hand_num, opp_label):
    ...
    for _ in range(200):
        ...
        who = "METAV5" if act_p == 0 else opp_label
    ...
    logf.write(f"  RESULT: METAV5 ev={reward[0]:+d} {opp_label} ev={reward[1]:+d}"
...
def run_match_logged(match_num, logf, OPP, opp_label):
    ...
    total_m5, total_opp, errors = 0, 0, 0
    ...
    return total_m5, total_opp, errors
...
def run_tournament(OPP, opp_label, log_file):
    ...
    with open(log_file, "w") as logf:
        logf.write(f"METAV5 vs {opp_label} — {MATCHES} matches x {HANDS_PER_MATCH} hands\n")
        logf.write("=" * 80 + "\n\n")
        ...
        for m in range(1, MATCHES + 1):
            ...
            r_m5, r_opp, errs = run_match_logged(m, logf, OPP, opp_label)
            ...
            print(f"  Match {m}: METAV5 {r_m5:+d}  {opp_label} {r_opp:+d}{err_s}"
                  f"  ({elapsed:.0f}s, total {total_elapsed:.0f}s)")

            logf.write(f"{'='*80}\n")
            logf.write(f"MATCH {m} SUMMARY: METAV5 {r_m5:+d}  {opp_label} {r_opp:+d}{err_s}\n")
            logf.write(f"{'='*80}\n\n")
    ...
    for i, (em, ea) in enumerate(zip(m5_evs, opp_evs), 1):
        print(f"    Match {i}: METAV5 {em:+d}  {opp_label} {ea:+d}")
