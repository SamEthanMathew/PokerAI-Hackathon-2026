...
# On the Jetson, run from repo root:  ./apps/poker-rl-trainer/run_training_detached.sh
# Or:  nohup ./apps/poker-rl-trainer/run_training_detached.sh &
# Then you can close your laptop; training continues on the Jetson.

set -e
cd "$(dirname "$0")/../.."
LOG_DIR="apps/poker-rl-trainer/logs"
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/train_detached.log"
echo "Starting training at $(date). Log: $LOG"
exec nohup .venv/bin/python apps/poker-rl-trainer/train.py --phase all >> "$LOG" 2>&1 &
