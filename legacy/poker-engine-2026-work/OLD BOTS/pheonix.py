...
class PlayerAgent(Agent):
    ...
    def __init__(self, stream: bool = True):
        ...
        self._binary_nodes["fold_to_steal_bb"] = self.BinaryNodeEstimator(
            prior=0.50, alpha0=2.0, beta0=2.0, reliability=0.85
        )
        self._binary_nodes["opp_3bet_sb"] = self.BinaryNodeEstimator(
            prior=0.15, alpha0=1.0, beta0=3.0, reliability=0.90
        )
        ...
        self._continuous_nodes = {
            ...
            "bluff_freq_large_bet": self.ContinuousNodeEstimator(prior=0.30, ema_decay=0.70, reliability=0.80),
            "bluff_freq_small_bet": self.ContinuousNodeEstimator(prior=0.30, ema_decay=0.70, reliability=0.80),
        }
        ...
        self._recent_steal_attempts = collections.deque(maxlen=5)
    ...
    def _process_opponent_action(self, observation, opp_action, last_was_bet, last_street):
        ...
        if last_street == 0 and opp_action in ("raise", "call", "check", "fold"):
            ...
            if self._last_position == "SB" and line_we_bet:
                self._node_binary("fold_to_steal_bb", opp_action == "fold")
                if opp_action == "fold":
                    self._recent_steal_attempts.append(True)
                elif opp_action == "raise":
                    self._node_binary("opp_3bet_sb", True)
                    self._recent_steal_attempts.append(False)
                elif opp_action == "call":
                    self._node_binary("opp_3bet_sb", False)
                    self._recent_steal_attempts.append(False)
    ...
    def _compute_exploit_adjustments(self, profile, live_stage=1):
        ...
        # Counter-Exploit: Check if they are punishing our steals
        opp_3bet_rate = profile.get("opp_3bet_sb", 0.15)
        recent_punishment = len(self._recent_steal_attempts) >= 2 and not any(list(self._recent_steal_attempts)[-2:])

        if opp_3bet_rate > 0.25 or recent_punishment:
            # They caught on to our steals, or the last two steals failed. Hard-stop preflop pressure.
            raw["preflop_pressure_adj"] = -0.10 
        else:
            bb_fold_rate = profile.get("fold_to_steal_bb", 0.5)
            if bb_fold_rate > 0.60:
                raw["preflop_pressure_adj"] += 0.15

        # MDF vs Exploit Priority
        # 'bluff_freq_large_bet' now inherently weights recent history heavier due to the faster EMA decay
        recent_large_bet_bluff_rate = profile.get("bluff_freq_large_bet", 0.3)

        # Only apply extreme overrides if confidence is high
        if profile.get("conf_bluff_freq_large_bet", 0.0) > 0.65:
            if recent_large_bet_bluff_rate > 0.40:
                raw["bluff_catch_adj"] += 0.20
                raw["hero_fold_adj"] -= 0.15
            elif recent_large_bet_bluff_rate < 0.15:
                raw["hero_fold_adj"] += 0.25
                raw["bluff_catch_adj"] -= 0.20
    ...
    def _learn_from_showdown(self, observation, info):
        ...
        if len(self._last_community) >= 5:
            ...
            for i in range(len(self._event_store) - 1, -1, -1):
                ...
                # Record bluff frequency by bet size bucket
                if ev.get("opp_action") in ("bet", "raise"):
                    bluff_val = 1.0 if revealed_bluff else 0.0
                    if s_bucket == "large":
                        self._node_continuous("bluff_freq_large_bet", bluff_val, ev.get("street", 3))
                    elif s_bucket == "small":
                        self._node_continuous("bluff_freq_small_bet", bluff_val, ev.get("street", 3))
    ...
    def _determine_gear(self, hands_left, ema_slope):
        ...
        time_weight = (hands_left / TOTAL_HANDS) ** 1.5
        ...
        if target_gear == "PRESSURE" and self._current_gear == "CHAOS":
            if self._running_pnl < self._gear_entry_pnl + 15:
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        if is_new_hand and not self._in_hand:
            ...
            if hands_left > 0:
                ...
                target_gear = self._determine_gear(hands_left, slope)
                
                time_weight = (hands_left / TOTAL_HANDS) ** 1.5
                cap = 1.5 if hands_left > 70 else (5.0 if hands_left < 30 else 1.5 + (5.0 - 1.5) * ((70 - hands_left) / 40.0))
                capped_slope = max(-cap, min(cap, slope))
                projected_final = self._running_pnl + (capped_slope * hands_left * time_weight)
                
                gear_priority = {"STANDARD": -1, "SNIPER": 0, "PRESSURE": 1, "CHAOS": 2}
                target_prio = gear_priority.get(target_gear, -1)
                current_prio = gear_priority.get(self._current_gear, -1)
        ...
        if street == 0:
            ...
            if self._current_gear == "CHAOS":
                if hands_left < 15:
                    # Push/Fold Mode - ignore postflop sizing, just jam or fold
                    # Hard Equity Floor to prevent pure RNG punting
                    opp_fold_rate = profile.get("fold_medium_flop", 0.5)
                    push_fold_eq_gate = 0.40
                    if self._last_position == "SB" and opp_fold_rate > 0.60:
                        push_fold_eq_gate = 0.30
                        
                    if preflop_eq >= push_fold_eq_gate:
                        if valid[RAISE]:
                            result = (RAISE, max_raise, 0, 0)
                            self._preflop_reason = "chaos_push_fold_jam"
                        elif valid[CALL]:
                            result = (CALL, 0, 0, 0)
                            self._preflop_reason = "chaos_push_fold_call"
                    else:
                        if valid[CHECK] and to_call <= 0:
                            result = (CHECK, 0, 0, 0)
                            self._preflop_reason = "chaos_push_fold_check"
                        elif valid[FOLD]:
                            result = (FOLD, 0, 0, 0)
                            self._preflop_reason = "chaos_push_fold_fold"
                elif valid[RAISE]:
        ...
        # ── Post-flop (streets 1-3) ──────────────────────────────────────────
            ...
            # Gear-based Gate Modifiers for Full-Street Consistency
            if self._current_gear == "CHAOS":
                opp_fold_rate = profile.get("fold_medium_flop", 0.5)
                has_ace_blocker = any(_RANK[c] == 8 for c in my_cards)
                
                if opp_fold_rate > 0.55:
                    strong_gate -= 0.10
                    good_gate -= 0.10
                else:
                    local_rng = random.Random(hash(str(self._hand_idx) + "blocker_bluff"))
                    is_bluffing_this_hand = local_rng.random() < 0.15
                    if has_ace_blocker and is_bluffing_this_hand:
                        strong_gate -= 0.10
                        good_gate -= 0.10
            ...
            # Sanity Floor for call_gate to prevent Double-Counting risk
            sanity_floor = 0.25 if to_call >= pot_ref * 0.8 else 0.15
            call_gate = max(call_gate, sanity_floor)
            ...
            if result is None:
                if self._current_gear == "CHAOS" and equity > strong_gate and valid[RAISE]:
                    # Stack-Awareness / Effective Stack Check
                    if max_raise > pot_ref * 1.5:
                        result = (RAISE, _clamp(int(pot_ref * 2.0), min_raise, max_raise), 0, 0)
                    else:
                        result = (RAISE, max_raise, 0, 0)
