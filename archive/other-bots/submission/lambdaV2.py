# LambdaV2: Meta chassis × Alpha killer instinct — ALL FLAWS FIXED
#
# Changes from LambdaV1:
#   1.  GRADIENT mode selection — weighted blend, not binary switches
#   2.  Flush draws / OESDs NO LONGER folded on flop (was critical bug)
#   3.  Equity penalty cap — total adjustments capped at -0.15
#   4.  Semi-bluff uses RAW equity, not penalty-adjusted
#   5.  River call cap varies by hand category (trips/2pair no longer fold)
#   6.  One-pair raise conditions loosened (actually fires now)
#   7.  Bleed-out skips discard computation (instant return)
#   8.  Early phase: first 3 hands use GTO, then AGRO-lite
#   9.  Time default 500s (was 600)
#  10.  Transition smoothing from early→normal phase
#  11.  Draw-aware postflop cap (flop draws call, turn draws check pot odds)
#  12.  River bluff tightened (only when opponent checks AND folds enough)

import os
import sys

# When loaded as submission.lambdaV2, ensure archive/other-bots is on path so "support" is findable
_other_bots_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
...
try:
    from support.libratus_tables import POLICY, KEEP_EQUITY, POSTERIOR, MATCHUPS
except ImportError:
    from libratus_tables import POLICY, KEEP_EQUITY, POSTERIOR, MATCHUPS

try:
    from support.discard_engine import choose_keep_postflop as _choose_keep_postflop
except ImportError:
...
ONE_PAIR_RAISE_EQ    = _PROFILE.get("one_pair_raise_equity",      0.55)
ONE_PAIR_RAISE_FOLD  = _PROFILE.get("one_pair_raise_fold_thresh",  0.35)
ONE_PAIR_RAISE_AGGRO = _PROFILE.get("one_pair_raise_aggro_cap",    0.45)  # was 0.35
ONE_PAIR_FOLD_PRESS  = _PROFILE.get("one_pair_fold_pressure",      0.55)  # was 0.50

RIVER_CALL_POT_RATIO_MAX = _PROFILE.get("river_call_pot_ratio_max", 0.35)

# Equity penalty cap — total adjustments from board/threat/discard never exceed this
MAX_EQUITY_PENALTY = _PROFILE.get("max_equity_penalty", 0.15)
...
class PlayerAgent(Agent):
    ...
    def __init__(self, stream: bool = True):
        ...
        self.logger.info(
            f"[LambdaV2 INIT] profile={_profile_name} "
    ...
    def _sim_budget(self, time_left):
        ...
        if time_left > 350:   return 200, 400
        elif time_left > 200: return 150, 300
        elif time_left > 100: return 80,  180
        elif time_left > 40:  return 50,  120
        return 25, 60
    ...
    def _process_opponent_action(self, observation, opp_action, last_was_bet, last_street):
        ...
        if not last_was_bet and opp_action == "raise":
            ...
            if self._hand_override is None:
                ...
                self.logger.info(
                    f"  [OVERRIDE H{self.hand_number} S{last_street}] "
                    f"reraise -> VALUE (cr={self._fmt_stat('check_raise')})"
                )
    ...
    def _select_mode(self):
        ...
        LambdaV2: TRUE GRADIENT — computes a weight [0,1] for each mode,
        then picks probabilistically. No more binary switches.
        
        Early phase (hands 1-3): GTO warmup to gather baseline stats.
        Early phase (hands 4-EARLY_PHASE_HANDS): AGRO-lite with growing mult,
            but can override to TRAP/VALUE on extreme evidence.
        Post early: full gradient mode selection.
        """
        ...
        # ── First 3 hands: GTO warmup to gather baseline stats ───────────
        if self.hand_number <= 3:
            self._mode_reason = "warmup_gto"
            return MODE_GTO, 1.0

        # ── Early phase (4 to EARLY_PHASE_HANDS): AGRO-lite ──────────────
        ...
        # ── Post early: GRADIENT mode selection ───────────────────────────
        # Compute a weight [0,1] for TRAP, VALUE, and AGRO.
        # Then pick mode probabilistically.

        # TRAP weight: driven by opponent aggression and check-raise rate
        trap_w = _clamp((opp_aggro - 0.25) / 0.30, 0.0, 1.0)  # 0 at agg=0.25, 1 at agg=0.55
        if cr_rate > 0.08:
            trap_w = max(trap_w, _clamp((cr_rate - 0.05) / 0.10, 0.0, 0.8))

        # VALUE weight: driven by low fold rate (calling station) or high call-down
        station_w = _clamp((0.38 - fold_to_bet) / 0.18, 0.0, 1.0)  # 0 at fb=0.38, 1 at fb=0.20
        calldown_w = _clamp((call_down - 0.40) / 0.25, 0.0, 1.0)   # 0 at cd=0.40, 1 at cd=0.65
        value_w = max(station_w, calldown_w)

        # AGRO weight: baseline + boost for overfolders and passive opponents
        agro_w = 0.3  # baseline
        if fold_to_bet > 0.40:
            agro_w += _clamp((fold_to_bet - 0.40) / 0.20, 0.0, 0.5)  # +0.5 at fb=0.60
        if opp_aggro < 0.20:
            agro_w += _clamp((0.20 - opp_aggro) / 0.15, 0.0, 0.3)    # +0.3 at agg=0.05
        if pf_raise > 0.45:
            agro_w += 0.15

        # Normalize weights
        total_w = trap_w + value_w + agro_w
        if total_w < 0.01:
            trap_w, value_w, agro_w = 0.0, 0.0, 1.0
            total_w = 1.0

        trap_p  = trap_w / total_w
        value_p = value_w / total_w
        agro_p  = agro_w / total_w

        # Compute agro multiplier (higher for overfolders/passive)
        ramp = min(1.0, (self.hand_number - EARLY_PHASE_HANDS) / 20.0)
        mult = 1.0
        if fold_to_bet > 0.50:
            mult = round(1.0 + 0.4 * ramp, 2)
        elif fold_to_bet > 0.40:
            mult = round(1.0 + 0.25 * ramp, 2)
        elif opp_aggro < 0.15 and self._stats["opp_aggression"][1] >= 8:
            mult = round(1.0 + 0.3 * ramp, 2)

        # ── Transition smoothing from early phase ────────────────────────
        if self.hand_number <= EARLY_PHASE_HANDS + 50:
            t_frac = 1.0 - (self.hand_number - EARLY_PHASE_HANDS) / 50.0
            bonus  = (EARLY_AGRO_MULT - 1.0) * max(t_frac, 0.0)
            mult   = round(mult + bonus, 2)

        # Pick mode by strongest weight (deterministic — same profile = same mode)
        if trap_p >= value_p and trap_p >= agro_p:
            self._mode_reason = f"gradient_trap({trap_p:.0%},agg={opp_aggro:.2f},cr={cr_rate:.2f})"
            return MODE_TRAP, 1.0
        elif value_p >= agro_p:
            self._mode_reason = f"gradient_value({value_p:.0%},fb={fold_to_bet:.2f},cd={call_down:.2f})"
            return MODE_VALUE, 1.0
        else:
            self._mode_reason = f"gradient_agro({agro_p:.0%},fb={fold_to_bet:.2f},x{mult:.1f})"
            return MODE_AGRO, mult
    ...
    def _keep_score_structural(self, keep2, community, opp_discards):
        """Non-MC structural heuristic — used for initial ranking in _choose_keep
        so the MC overlay only fires 3 times (not 10+3=13)."""
        pp  = self._pressure_potential(keep2, community, opp_discards)
        dd  = self._draw_density(keep2, community)
        mhv = self._made_hand_value(keep2, community)
        bv  = self._blocker_value(keep2, community, opp_discards)
        sf  = self._story_flexibility(keep2, community)
        return _KW_PP*pp + _KW_DD*dd + _KW_MHV*mhv + _KW_BV*bv + _KW_SF*sf
    ...
    def _choose_keep(self, my_cards, community, opp_discards, d_sims, mode=MODE_AGRO):
        ...
        if community and len(community) >= 3:
            ...
            if len(my_cards) == 5:
                # Step 1: structural (non-MC) score for all 10 combos — no MC calls here
                heuristic_scores = []
                for i, j in combinations(range(5), 2):
                    keep = [my_cards[i], my_cards[j]]
                    sc   = self._keep_score_structural(keep, community, opp_discards)
                    heuristic_scores.append((i, j, sc))
                # Normalise heuristic scores to [0,1] range
                sc_vals = [sc for _, _, sc in heuristic_scores]
                sc_min, sc_max = min(sc_vals), max(sc_vals)
                sc_range = (sc_max - sc_min) if sc_max > sc_min else 1.0
                # Step 2: Top-3 by structural heuristic
                top3 = sorted(heuristic_scores, key=lambda x: x[2], reverse=True)[:3]
                # Step 3: MC equity only on top-3 (3 MC calls, not 13)
                best_blend    = -1.0
    ...
    def _agro_postflop(self, street, my_cards, community, opp_discards, my_discards,
        ...
        # Save RAW equity before any penalties (for semi-bluff check)
        raw_equity = equity

        # Apply penalties with a CAP (Fix #11)
        penalty = 0.0
        if _board_paired_and_we_weak(my_cards, community):
            penalty += 0.08
        penalty += _alpha_discard_threat(opp_discards, community)
        penalty = min(penalty, MAX_EQUITY_PENALTY)  # cap total penalty
        equity -= penalty
        ...
        # ── Semi-bluff band (Alpha edge): RAW equity 0.38-0.52 on flop/turn ──
        # Uses raw_equity (Fix #5) — not the penalty-adjusted version.
        if street in (1, 2):
            ...
            if (SEMI_BLUFF_LO <= raw_equity <= SEMI_BLUFF_HI and
    ...
    def _value_postflop(self, street, my_cards, community, opp_discards, my_discards,
        ...
        equity = self._mc_equity(my_cards, community, dead, num_sims=p_sims) \
                 if len(my_cards) == 2 and len(community) >= 3 else 0.45
        # Apply penalties with cap (Fix #11)
        penalty = 0.0
        if _board_paired_and_we_weak(my_cards, community):
            penalty += 0.08
        penalty += _alpha_discard_threat(opp_discards, community)
        penalty = min(penalty, MAX_EQUITY_PENALTY)
        equity -= penalty
    ...
    def _river_action(self, mode, my_cards, community, opp_discards, my_discards,
        ...
        # ── River call cap by hand category (Fix #9) ────────────────────
        # Stronger hands tolerate bigger river bets
        river_cap = RIVER_CALL_POT_RATIO_MAX  # default 0.35
        if hand_cat in ("flush", "full_house"):
            river_cap = 1.0   # call any size
        elif hand_cat == "straight":
            river_cap = 0.70
        elif hand_cat == "trips":
            river_cap = 0.60  # was 0.35
        elif hand_cat == "two_pair":
            river_cap = 0.50  # was 0.35

        if to_call > pot_ref * river_cap:
            if not is_prem_pair:
                if valid[FOLD]:  return (FOLD, 0, 0, 0)
                if valid[CHECK]: return (CHECK, 0, 0, 0)
            ...
            if equity > 0.65:
                if valid[RAISE] and max_raise >= min_raise:
                    amt = _clamp(int(pot_ref * self._rng.uniform(0.70, 0.90)),
                                 min_raise, max_raise)
                    return (RAISE, amt, 0, 0)
            if to_call > 0 and equity > pot_odds and to_call <= pot_ref * river_cap:
                if valid[CALL]: return (CALL, 0, 0, 0)
            if valid[CHECK]: return (CHECK, 0, 0, 0)
            return (FOLD, 0, 0, 0)
        ...
        if hand_cat == "one_pair":
            ...
            if to_call > 0 and equity > pot_odds and to_call <= pot_ref * river_cap:
                # Additional discipline: don't call with marginal one-pair vs station
                if fold_rate >= 0.35 and call_down < 0.50:
                    if valid[CALL]: return (CALL, 0, 0, 0)
            if valid[CHECK]: return (CHECK, 0, 0, 0)
            return (FOLD, 0, 0, 0)
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        time_left    = observation.get("time_left",       500.0)
        ...
        if is_new_hand:
            engine_hand = info.get("hand_number")
            if engine_hand is not None:
                self.hand_number = engine_hand  # sync to engine (authoritative)
            else:
                self.hand_number += 1
        ...
        # ── Postflop guardrails (flop/turn only — river handled above) ─────
        # Fix #4:  Draws (flush_draw, oesd) CALL on flop, check pot odds on turn
        # Fix #10: One-pair raise conditions loosened
        if street != 3 and len(my_cards) == 2 and len(community) >= 3:
            hand_cat    = _hand_rank_category(my_cards, community)
            to_call_now = max(0, opp_bet - my_bet)
            pot_ref_now = max(pot_size, 1)
            pot_odds_now = to_call_now / (pot_ref_now + to_call_now) if (pot_ref_now + to_call_now) > 0 else 0

            if hand_cat == "one_pair":
                fold_to_bet = self._safe_rate("fold_to_bet")
                opp_aggro_r = self._safe_rate("opp_aggression")
                # One-pair raise: top pair + opponent folds enough + not too aggressive + not extremely wet
                can_raise = (
                ...
                if result[0] == RAISE and not can_raise:
                    if to_call_now > 0 and valid[CALL]:
                        ...
                        result = (CHECK, 0, 0, 0)
                # Fold one pair facing big pressure
                if result[0] == CALL and to_call_now > pot_ref_now * ONE_PAIR_FOLD_PRESS:
                    if valid[FOLD]:
                        result = (FOLD, 0, 0, 0)

            elif hand_cat in ("flush_draw", "oesd"):
                # Fix #4: DRAWS SURVIVE — call if pot odds justify, never auto-fold
                if to_call_now > 0:
                    if street == 1:
                        # Flop: 2 cards to come, draws have ~45-50% equity — always call reasonable bets
                        if to_call_now <= pot_ref_now * 0.60 and valid[CALL]:
                            result = (CALL, 0, 0, 0)
                        elif self._last_equity > pot_odds_now and valid[CALL]:
                            result = (CALL, 0, 0, 0)
                        elif valid[FOLD]:
                            result = (FOLD, 0, 0, 0)
                    else:
                        # Turn: 1 card to come, need better pot odds
                        if self._last_equity > pot_odds_now + 0.05 and valid[CALL]:
                            result = (CALL, 0, 0, 0)
                        elif to_call_now <= pot_ref_now * 0.30 and valid[CALL]:
                            result = (CALL, 0, 0, 0)
                        elif valid[FOLD]:
                            result = (FOLD, 0, 0, 0)
                # If no bet to call, keep original action (check or small raise is fine)

            elif hand_cat == "gutshot":
                # Gutshot: call small bets on flop, fold on turn unless cheap
                if to_call_now > 0:
                    if street == 1 and to_call_now <= pot_ref_now * 0.35 and valid[CALL]:
                        result = (CALL, 0, 0, 0)
                    elif valid[FOLD]:
                        result = (FOLD, 0, 0, 0)
                elif valid[CHECK]:
                ...
                # No made hand, no draw — fold to any bet, check if free
                if to_call_now > 0:
                    if valid[FOLD]:
                        result = (FOLD, 0, 0, 0)
                elif valid[CHECK]:
        ...
        if result[0] in (RAISE, CALL, FOLD):
            act_name = {RAISE: "RAISE", CALL: "CALL", FOLD: "FOLD"}[result[0]]
    ...
    def observe(self, observation, reward, terminated, truncated, info):
        ...
        if terminated:
            ...
            pnl_hand = int(reward)
            sign = "+" if pnl_hand >= 0 else ""
            mode = self._hand_override or self._hand_mode
            self.logger.info(
                f"  [END H{self.hand_number} {mode}] "
                f"result={sign}{pnl_hand} running={self._running_pnl:+d}"
            )