...
temp_dir="aipoker"
rm -rf "$temp_dir"
mkdir -p "$temp_dir/agents" "$temp_dir/submission" "$temp_dir/docs" "$temp_dir/config"

# Copy files and folders to temporary directory, maintaining structure
cp gym_env.py match.py run.py requirements.txt "$temp_dir/"
cp tests/agent_test.py "$temp_dir/agent_test.py"
...
cp config/agent_config.json "$temp_dir/config/"
