...
class SessionLogger:
    ...
    def log_decision(self, hand_number: int, street: str, active_team: int, team_0_bankroll: int, team_1_bankroll: int,
        team_0_cards: list, team_1_cards: list, board_cards: list, team_0_discarded: list, team_1_discarded: list,
        team_0_bet: int, team_1_bet: int, action_type: str, action_amount: int, action_keep_1: int, action_keep_2: int,
        actor: str, decision_time_sec: float = 0.0, human_reason: str = "", human_confidence: str = "", human_read: str = "",
        human_discard_reason: str = "", opp_keep_inference: str = "", valid_actions: list | None = None,
        cards_known_count: int = 0, cards_known_list: list | None = None, bot_recommendation_keep: str = "", is_discard: bool = False):
        if self._writer is None:
            return
        row = {
            "hand_number": hand_number, "street": street, "active_team": active_team,
            "team_0_bankroll": team_0_bankroll, "team_1_bankroll": team_1_bankroll,
            ...
            "team_0_bet": team_0_bet, "team_1_bet": team_1_bet,
            "action_type": action_type, "action_amount": action_amount, "action_keep_1": action_keep_1, "action_keep_2": action_keep_2,
            "actor": actor,
            "decision_time_sec": round(decision_time_sec, 3) if actor == "Human" else "",
            "time_used_player_0": round(self._time_used_0, 3), "time_used_player_1": round(self._time_used_1, 3),
            "human_reason": human_reason, "human_confidence": human_confidence, "human_read": human_read,
            "human_discard_reason": human_discard_reason, "opp_keep_inference": opp_keep_inference,
            "valid_actions": json.dumps(valid_actions) if valid_actions is not None else "",
            "cards_known_count": cards_known_count, "cards_known_list": _serialize_cards(cards_known_list or []),
    ...
    def add_time_used(self, player: int, sec: float):
        if player == 0: self._time_used_0 += sec
        else: self._time_used_1 += sec

    def log_rl_transition(self, hand_id: int, step_in_hand: int, player: int, obs: dict, action: list | tuple, reward: float, done: bool, derived_state: dict | None, is_human: bool, info: dict | None = None):
        o = {k: (v.tolist() if hasattr(v, "tolist") else v) for k, v in obs.items()}
        for k, v in list(o.items()):
            if hasattr(v, "item"): o[k] = v.item()
            elif isinstance(v, (list, tuple)) and v and hasattr(v[0], "item"): o[k] = [x.item() if hasattr(x, "item") else x for x in v]
        ds = derived_state or {}
        ds_serializable = {k: (v.tolist() if hasattr(v, "tolist") else v) for k, v in ds.items()}
        rec = {"hand_id": hand_id, "step_in_hand": step_in_hand, "player": player, "obs": o, "action": list(action), "reward": reward, "done": done, "derived_state": ds_serializable, "is_human": is_human, "info": info or {}}
    ...
    def end_session(self, total_hands: int, bankroll_0: int, bankroll_1: int, bot_name: str):
        if self._csv_file: self._csv_file.close(); self._csv_file = None
        summary = {"session_id": self.session_id, "bot_name": bot_name, "total_hands": total_hands, "bankroll_0": bankroll_0, "bankroll_1": bankroll_1, "time_used_0": round(self._time_used_0, 2), "time_used_1": round(self._time_used_1, 2), "post_hand_annotations": self._post_hand_annotations}
