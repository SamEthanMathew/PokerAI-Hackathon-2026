...
class PlayerAgent(Agent):
    ...
    def observe(self, observation, reward, terminated, truncated, info):
        ...
        if terminated:
            ...
            self.logger.info(
                f"HAND_RESULT | hand={self._hands_completed} | reward={int(reward)} | pnl={self._running_pnl}")
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        if not valid[DISCARD]:
            ...
            if self._running_pnl > max_bleed:
                if valid[FOLD]:
                    ...
                    self.logger.info(f"ACT h={self._hands_completed} s={street} FOLD rules={self._rules_fired} dt={(time.time()-_act_t0)*1000:.0f}ms")
                    return (FOLD, 0, 0, 0)
                if valid[CHECK]:
                    self._rules_fired.append("bleed_lock")
                    self.logger.info(f"ACT h={self._hands_completed} s={street} CHECK rules={self._rules_fired} dt={(time.time()-_act_t0)*1000:.0f}ms")
                    return (CHECK, 0, 0, 0)
        ...
        if valid[DISCARD]:
            ...
            self.logger.info(f"ACT h={self._hands_completed} s={street} DISCARD keep={best_ij} dt={(time.time()-_act_t0)*1000:.0f}ms")
            return _disc_result
        ...
        # ── Post-flop (streets 1-3) ──────────────────────────────────────────
            ...
            if (to_call >= 30 and hand_cat in ("nothing", "one_pair", "two_pair")
        ...
        self.logger.info(f"ACT h={self._hands_completed} s={street} {_a}{_extra} eq={self._act_equity} cat={self._act_hand_cat} rules={self._rules_fired} dt={(time.time()-_act_t0)*1000:.0f}ms")
        return result
