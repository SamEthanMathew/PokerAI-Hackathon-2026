"""
GenesisV1 — Best-of-both bot.

Combines Omicron's proven core algorithms with Genesis's logging infrastructure:
  • Exact brute-force discard equity  (replaces Genesis thin MC)
  • Adaptive opponent discard model   (replaces Genesis nothing)
  • Range-weighted postflop equity    (replaces Genesis simple MC)
  • 4-archetype profiling + dynamic sizing  (replaces Genesis VPIP/PFR only)
  • Bleed-out protection lock         (new)
  • Explicit semi-bluff EV calculation (replaces Genesis heuristic)
  • Board texture equity penalties    (replaces Genesis minimal)

Root causes addressed (from M1 log analysis):
...
class GenesisV1Agent(Agent):
    """
    Best-of-both agent: Omicron algorithms + Genesis logging.
    """
    ...
    def __init__(self, stream: bool = True, entry_point: str = "genesisV1"):
    ...
    def _act_street0(self, my_cards, my_bet, opp_bet, valid, min_raise, max_raise):
        ...
        if in_early:
            ...
            if premium:
                ...
                pf_eq = self._preflop_equity(my_cards)
                if pf_eq >= NORMAL_PREFLOP_MIN_EQUITY:
    ...
    def _act_postflop(self, my_cards, community, opp_discards, my_discards,
        ...
        strength = self._cat_to_strength(hand_cat, has_draw)

        # Apply board texture adjustments
        outs = max(flush_outs, str_outs)
        if street <= 2 and has_draw and outs > 0:
