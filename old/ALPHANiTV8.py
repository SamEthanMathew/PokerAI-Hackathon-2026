# ALPHANiTV8: Exact subgame solver for discard + adaptive opponent modeling
#              + full postflop engine with range-weighted equity, board texture
#              awareness, semi-bluff engine, dynamic sizing, and opponent profiling.
...
def _board_paired_and_we_weak(my_cards, community):
    ...
    if bp_rank in my_ranks:
        return False
    if my_ranks[0] == my_ranks[1] and frozenset([my_ranks[0], my_ranks[0]]) in PREMIUM_PAIRS:
        return False
    return True
...
class _OppDiscardModel:
    ...
    def _extract_features(keep, flop, _fr=None, _fs=None, _bsc=None):
        ...
        all_suits = [s0, s1] + flop_suits
        sc = Counter(all_suits)
        flush_draw = 1.0 if sc.most_common(1)[0][1] >= 4 else 0.0
        ...
        board_suit_count = _bsc if _bsc is not None else Counter(flop_suits)
        bsm = sum(1 for c in keep if board_suit_count.get(_suit(c), 0) >= 2) / 2.0
...
class PlayerAgent(Agent):
    ...
    def _exact_discard_equity(self, my_keep, community, dead_cards):
        ...
        for runout in combinations(remaining, board_needed):
            ...
            for opp in combinations(opp_pool, 2):
                oh = [_INT_TO_TREYS[opp[0]], _INT_TO_TREYS[opp[1]]]
                oha = [_INT_TO_TREYS_ALT[opp[0]], _INT_TO_TREYS_ALT[opp[1]]]
    ...
    def _opp_keep_weight(self, opp_hand, community, opp_discards, _flop_cache=None):
        ...
        for i, j in combinations(range(len(original5)), 2):
            ...
            keeps_and_scores.append((set([original5[i], original5[j]]), sc))
    ...
    def _exact_discard_equity_weighted(self, my_keep, community, dead_cards, opp_discards):
        ...
        flop = community[:3]
        flop_cache = ([_rank(c) for c in flop], [_suit(c) for c in flop],
                      Counter([_suit(c) for c in flop]))
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        if valid[DISCARD]:
            ...
            if have_opp_discards:
                ...
                candidates = []
                for i, j, keep, toss in all_keeps:
                    mc_eq = self._compute_equity(keep, community, opp_discards, toss, num_sims=500)
                    candidates.append((i, j, mc_eq, keep, toss))
