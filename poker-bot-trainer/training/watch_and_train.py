...
def train_bc(records: list[dict], gen: int, hidden_dim: int = 256, epochs: int = 30) -> str | None:
    """
    Train a PokerCloneNet on the given records.
    Returns path to saved checkpoint, or None if not enough data.
    """
    import torch.nn as nn
    from torch.utils.data import DataLoader, TensorDataset, random_split

    b_feats, b_labels, r_buckets, d_feats, d_labels = process_session_data(records)
    ...
    train_ds, val_ds = random_split(TensorDataset(B, BL, BR), [len(B) - n_val, n_val])
