# MetaBot v6 (METAV6): Adaptive Decision-Tree Poker Agent (fork of METAV5)
#
...
# pairs, matching the empirical finding that MetaV5/METAV6 kept flush draws only 498
...
def _board_monotone_penalty(my_cards, community):
    """Equity penalty when 3+ community cards share a suit and we lack that suit."""
    if len(my_cards) < 2 or len(community) < 3:
        return 0.0
    sc = Counter(_suit(c) for c in community)
    dom_suit, dom_cnt = sc.most_common(1)[0]
    if dom_cnt < 3:
        return 0.0
    my_in_suit = sum(1 for c in my_cards[:2] if _suit(c) == dom_suit)
    if my_in_suit == 0:
        return -0.18
    if my_in_suit == 1:
        return -0.06
...
def _board_connected_penalty(my_cards, community):
    """Equity penalty when board has 3+ sequential ranks and we only have trips/two-pair."""
    if len(my_cards) < 2 or len(community) < 3:
        return 0.0
    b_ranks = [_rank(c) for c in community]
...
def _our_flush_rank(my_cards, community):
    """Return 'high'/'low'/None indicating our flush rank contribution.

    'high' = our best hole card in the flush suit is rank 7+ (8/9/A).
    'low'  = we have a flush but contribute only low cards.
    None   = we don't have a flush.
    """
    if len(my_cards) < 2 or len(community) < 3:
        return None
    all_suits = [_suit(c) for c in my_cards[:2]] + [_suit(c) for c in community]
    sc = Counter(all_suits)
    flush_suit, flush_cnt = sc.most_common(1)[0]
    if flush_cnt < 5:
        return None
    my_suit_ranks = [_rank(c) for c in my_cards[:2] if _suit(c) == flush_suit]
    if not my_suit_ranks:
        return None
    if max(my_suit_ranks) >= 7:
        return "high"
    return "low"


def _trips_on_board_kicker(my_cards, community):
    """Detect trips on the board itself and evaluate our kicker strength.

    Returns 'weak_kicker' / 'strong_kicker' / None.
    """
    if len(my_cards) < 2 or len(community) < 3:
        return None
    b_ranks = Counter(_rank(c) for c in community)
    trip_rank = next((r for r, cnt in b_ranks.items() if cnt >= 3), None)
    if trip_rank is None:
        return None
    my_ranks = sorted([_rank(c) for c in my_cards[:2]], reverse=True)
    best_kicker = my_ranks[0]
    if best_kicker <= 3:
        return "weak_kicker"
    return "strong_kicker"


def _opp_flush_inference(community, opp_discards):
    """Equity penalty when opponent discards suggest they kept a flush suit."""
    if not opp_discards or len(opp_discards) < 3 or len(community) < 3:
        return 0.0
    b_sc = Counter(_suit(c) for c in community)
    opp_suits = [_suit(c) for c in opp_discards]
    for suit, cnt in b_sc.items():
        if cnt >= 2:
            opp_in_suit = sum(1 for s in opp_suits if s == suit)
            if opp_in_suit == 0:
                return -0.08
    return 0.0
...
class PlayerAgent(Agent):
    ...
    def _street_adjust(self, equity, street, has_draw, flush_outs, straight_outs,
                        ...
                        opp_discards=None):
        """Adjust raw MC equity for street-dependent implied odds, draw death,
        and board-texture penalties (monotone, connected, opp flush inference)."""
    ...
    # (max flush draw ~116), so MetaV5/METAV6 almost never kept flush draws.  The new
    ...
    def _choose_keep(self, my_cards, community, opp_discards, d_sims, mode=MODE_AGRO):
        ...
        for i, j in combinations(range(len(my_cards)), 2):
            ...
            for card in keep:
                if _rank(card) <= 3:
                    ace_in_toss = any(_rank(c) == RANK_A for c in toss)
                    higher_suit_in_toss = any(
                        _suit(c) == _suit(card) and _rank(c) > _rank(card) for c in toss
                    )
                    if ace_in_toss or higher_suit_in_toss:
                        total -= 15
    ...
    def _gto_action(self, street, my_cards, community, opp_discards, my_discards,
        ...
        if street >= 1 and len(my_cards) == 2 and len(community) >= 3:
            ...
            equity = self._street_adjust(equity, street, has_draw, f_outs, s_outs,
                                         hand_cat=_hcat, my_cards=my_cards,
                                         community=community, opp_discards=opp_discards)
    ...
    def _cat_to_strength(hand_cat, has_draw, my_cards=None, community=None):
        if hand_cat == "trips_plus":
            if my_cards and community:
                tbk = _trips_on_board_kicker(my_cards, community)
                if tbk == "weak_kicker":
                    return "medium"
                fr = _our_flush_rank(my_cards, community)
                if fr == "low":
                    return "strong"
            return "monster"
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        # ── Bleed-out lock: if folding every remaining hand still wins, fold/check every time (no exceptions)
        ...
        # ── River fold override on monotone boards ─────────────────────
        if street == 3 and result[0] != FOLD and len(my_cards) == 2 and len(community) >= 3:
            to_call_rv = max(0, opp_bet - my_bet)
            if to_call_rv > pot_size * 0.30:
                mono_pen = _board_monotone_penalty(my_cards, community)
                if mono_pen <= -0.15:
                    hcat_rv = _hand_rank_category(my_cards, community)
                    if hcat_rv not in ("trips_plus",):
                        if valid[FOLD]:
                            result = (FOLD, 0, 0, 0)
        ...
        if result[0] == RAISE and len(my_cards) == 2 and len(community) >= 3:
            ...
            _hcat = _hand_rank_category(my_cards, community)
            _str = self._cat_to_strength(_hcat, _has_d, my_cards=my_cards, community=community)
        ...
        # ── Enforce bleed-out: never return RAISE/CALL when we're in lock (safety net)
        rounds_left = max(0, TOTAL_HANDS - self.hand_number)
        sb_left = (rounds_left + 1) // 2
        bb_left = rounds_left - sb_left
        max_bleed = sb_left * 1 + bb_left * 2
